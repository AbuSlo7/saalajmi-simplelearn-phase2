...
export class ItemDetailsComponent implements OnInit {
  ...
  constructor(private details: DetailsService, private cart: CartService) {
    this.details.rate.subscribe(rate => this.rate = rate);
  ...
  AddCart(){
    if (Number(this.selected_quantity) > this.quantity || this.selected_quantity == '') {
      this.alert = '* The selected quantity is more than available in stock or empty';
      ...
      this.alert = '* Items have been added to the cart successfully';
